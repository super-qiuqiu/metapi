...
export function buildConfig(env: NodeJS.ProcessEnv) {
  ...
  return {
    ...
    codexUpstreamWebsocketSameAccountRetries: Math.max(0, Math.trunc(parseNumber(env.CODEX_UPSTREAM_WEBSOCKET_SAME_ACCOUNT_RETRIES, 1))),
    codexStickyAccountEnabled: parseBoolean(env.CODEX_STICKY_ACCOUNT_ENABLED, true),
    ...
    codexContextCompactionAutoEnabled: parseBoolean(env.CODEX_CONTEXT_COMPACTION_AUTO_ENABLED, false),
    codexContextCompactionSoftTokens: Math.max(1_000, Math.trunc(parseNumber(env.CODEX_CONTEXT_COMPACTION_SOFT_TOKENS, 50_000))),
    codexContextCompactionTargetTokens: Math.max(1_000, Math.trunc(parseNumber(env.CODEX_CONTEXT_COMPACTION_TARGET_TOKENS, 30_000))),
    codexContextCompactionCooldownTurns: Math.max(0, Math.trunc(parseNumber(env.CODEX_CONTEXT_COMPACTION_COOLDOWN_TURNS, 3))),
    codexContextCompactionUnsupportedTtlMs: Math.max(1_000, Math.trunc(parseNumber(env.CODEX_CONTEXT_COMPACTION_UNSUPPORTED_TTL_MS, 600_000))),
    codexContextCompactionMaxAttemptsPerSession: Math.max(0, Math.trunc(parseNumber(env.CODEX_CONTEXT_COMPACTION_MAX_ATTEMPTS_PER_SESSION, 8))),
    // Context window guard configuration (4-layer protection)
    contextWindowGuardEnabled: parseBoolean(env.CONTEXT_WINDOW_GUARD_ENABLED, true),
    contextWindowGuardAutoCompactPercent: Math.max(50, Math.min(99, Math.trunc(parseNumber(env.CONTEXT_WINDOW_GUARD_AUTO_COMPACT_PERCENT, 80)))),
    contextWindowGuardTrimTargetPercent: Math.max(40, Math.min(90, Math.trunc(parseNumber(env.CONTEXT_WINDOW_GUARD_TRIM_TARGET_PERCENT, 75)))),
