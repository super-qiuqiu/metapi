...
3. **After development complete**:
   ...
   - After testing passes, AI may commit/push only with explicit user consent in the current conversation; otherwise human commits manually
...
6. [!] Execute `git commit` / `git push` only with explicit user consent in the current conversation; without consent, AI must not perform commit/push actions
